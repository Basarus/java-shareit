...
class ItemRequestControllerTest {
    ...
    void create_shouldReturnCreatedRequest() throws Exception {
        ...
        ItemRequestDto response = ItemRequestDto.builder().id(1L).description("Нужен шуруповерт").created(LocalDateTime.now()).items(List.of()).build();

        Mockito.when(itemRequestService.create(eq(1L), any(ItemRequestDto.class))).thenReturn(response);
    ...
    void getOwnRequests_shouldReturnList() throws Exception {
        ...
        Mockito.when(itemRequestService.getUserRequests(1L)).thenReturn(List.of(r1, r2));
    ...
    void getAll_shouldReturnList() throws Exception {
        ...
        Mockito.when(itemRequestService.getAllRequests(1L, 0, 10)).thenReturn(List.of(r1));
    ...
    void getById_shouldReturnRequestWithItems() throws Exception {
        ...
        ItemRequestDto response = ItemRequestDto.builder().id(3L).description("Нужна отвертка").created(LocalDateTime.now()).items(List.of(itemDto)).build();

        Mockito.when(itemRequestService.getById(1L, 3L)).thenReturn(response);
