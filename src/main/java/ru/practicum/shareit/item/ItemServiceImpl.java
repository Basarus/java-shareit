...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto create(Long ownerId, ItemDto dto) {
        ...
        if (dto.getAvailable() == null) throw new
                BadRequestException("available must not be null");
        if (dto.getName() == null || dto.getName().isBlank()) throw new
                BadRequestException("name must not be blank");
    ...
    public ItemDto update(Long ownerId, Long itemId, ItemDto patch) {
        ...
        if (patch.getName() != null) {
            if (patch.getName().isBlank()) throw new
                    BadRequestException("name must not be blank");
            i.setName(patch.getName());
        }
        if (patch.getDescription() != null) {
            if (patch.getDescription().isBlank()) throw new
                    BadRequestException("description must not be blank");
